...
export default function TransactionsTab({ limit = 25, showViewAll = true }: TransactionsTabProps) {
  ...
  const openRefundModal = (tx: Transaction) => {
    setRefundTarget(tx);
    const preferNgn = currency === 'NGN' && !!exchangeRate;
    const defaultAmount = preferNgn
      ? (tx.amount_usd * (exchangeRate || 1)).toFixed(2)
  ...
  const submitQuickRefund = async () => {
    ...
    if (!isAmountlessNgnBankRefund && (!Number.isFinite(parsedAmount) || parsedAmount <= 0)) {
    ...
    try {
      await refundsApi.createForPayment(refundTarget.id, {
        ...
        amount_ngn: refundCurrency === 'ngn' && refundDestination !== 'bank_account' ? parsedAmount : undefined,
      ...
      setRefundMessage('Refund submitted successfully.');
      await loadData();
      setTimeout(() => {
        closeRefundModal();
      }, 650);
    } catch (error) {
      setRefundError(error instanceof Error ? error.message : 'Failed to submit refund');
    } finally {
    ...
    <div className="space-y-5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-16 text-center text-slate-400 dark:text-slate-500 text-sm">
      ...
      {refundTarget && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#11131d] p-4 space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                onClick={closeRefundModal}
                className="px-3 py-2 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-700"
              >
                Cancel
              </button>
              <button
                onClick={submitQuickRefund}
                disabled={isSubmittingRefund}
                className="px-3 py-2 text-xs font-semibold rounded-lg bg-primary text-white disabled:opacity-50"
              >
                {isSubmittingRefund ? 'Submitting...' : 'Submit Refund'}
              </button>
            </div>

            {refundMessage && <p className="text-xs text-emerald-600 dark:text-emerald-400">{refundMessage}</p>}
            {refundError && <p className="text-xs text-rose-600 dark:text-rose-400">{refundError}</p>}
          </div>
        </div>
      )}
    </div>
